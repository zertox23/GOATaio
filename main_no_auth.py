...
bl, ree, lr, wh, gr, cy, lb, res, ma, lm, lc, ye = (
    Fore.BLUE,
    Fore.RED,
    Fore.LIGHTRED_EX,
    Fore.WHITE,
        ...
        try:
            ...
            try:
                ...
                with open("errors.txt", "a", errors="ignore") as errors:
                    errors.write(
                        f"TIME:{str(datetime.datetime.now())[:-7].replace(':','-')}  ERORR HAPPENED:{E}\n"
                    )
...
def capture_remover():
    ...
    with open(
        os.getcwd() + "\\results\\CAPTURE_REMOVER\\" + namy,
        "a",
        encoding="utf-8",
        errors="ignore",
    ) as f:
    ...
    for line in combo:
        try:
            listt.append(
                str(line.split(":")[0])
                + ":"
                + str(line.split(":")[1]).strip().capitalize()
                + "!"
                + "\n"
            )
            listt.append(
...
def get_pagetypes():
    ...
    try:
        ...
        with open(
            os.getcwd() + "\\results\\SKIDED_PAGETYPES\\" + namy,
            "a",
            encoding="utf-8",
            errors="ignore",
        ) as f:
    ...
    def get_pageformats():
        ...
        with open(
            os.getcwd() + "\\results\\SKIDED_PAGEFORMATS\\" + namy,
            "a",
            encoding="utf-8",
            errors="ignore",
        ) as f:
    ...
    def domain_sorter():
        ...
        try:
            ...
            for item in dir:
                with open(
                    os.getcwd() + f"\\results\DOMAIN_SORTER\\" + item.strip(),
                    errors="ignore",
                ) as l:
                    k = l.readlines()
        ...
        plt.tight_layout()
        plt.subplots_adjust(
            top=0.969, bottom=0.495, left=0.08, right=0.977, hspace=0.2, wspace=0.2
        )
        plt.show()
    ...
    def check_domains():
        ...
        with open(
            os.getcwd() + "\\REQUIRMENTS\\DATA.txt",
            "r",
            encoding="utf-8",
            errors="ignore",
        ) as file:
        ...
        for line in combo:
            ...
            if domain.strip() in domainslst:
                ...
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"|GOAT-ALLL IN ONE|  |VERSION|:3.8   |MODULE|:(Domain Hunter) |CPS:{format(cps,',')}| |CPM: {format(cpm,',')}| |Hits: {format(x,',')}/{format(k,',')}|  |All: {format(p,',')}/{format(k,',')}| "
                )
                with open(
                    os.getcwd() + "\\results\\DOMAIN_HUNTER\\" + namy,
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            for line in output:
                if not any(bad_word in line for bad_word in bad_words):
                    if line.count(" ") > 1 and line.count(" ") < 3:
                        if file is None:
                            with open(
                                os.getcwd() + f"\\results\\KEYWORDS_FROM_URLS\\{namy}",
                                "a",
                                encoding="utf-8",
                                errors="ignore",
                            ) as f:
        ...
        for dork in dorks:
            ...
            if proxy_type == "1":
                for i in range(5):
                    ...
                    resp = session.get(
                        url=f"https://www.google.com/search?q={urllib.parse.quote(dork)}&num=50&start={i*10}"
                    )
...
def Filter_urls():
    ...
    with open(
        os.getcwd() + "\\results\\ClEANED_URLS\\" + namy,
        "a",
        encoding="utf-8",
        errors="ignore",
    ) as file:
...
def write_sqls(namye, lista, date):
    ...
    try:
        with open(
            os.getcwd() + "\\results\\SQL_HITS\\" + namy(namye),
            "a",
            encoding="utf-8",
            errors="ignore",
        ) as f:
...
def SQL_SCANNER():
    namy = (
        "[SQL - Hits] {" + str(datetime.datetime.now())[:-7].replace(":", "-") + "}.txt"
    )
    hits = []
    try:
        os.mkdir(os.getcwd() + "\\results")
            ...
            if "=" not in line:
                ...
                try:
                    ...
                    if "MySQL" in checker.text or "mysql" in checker.text:
                        ...
                        "native client" in checker.text
                        or "Native Client" in checker.text
                    ):
                        hits.append(line.strip())
                        ...
                        "syntax error" in checker.text or "Syntax Error" in checker.text
                    ):
                        hits.append(line.strip())
                ...
                PrintGradient("#00FFFF", "#FF69B4", NAME)
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"|GOAT-ALLL IN ONE|  |VERSION|:3.8   |MODULE|:(SQLI_SCANNER) STATS= CPM:{cpm}  Nothing:{nothing}   Nones: {Nonee}     MYSQL= {MySQL}   MsSQL: {MsSQL}   PostGRES: {PostGRES}   Oracle: {Oracle}   MariaDB: {MariaDb}    OVR(SQLS): {sqls}    ERORRS: {Errorr}   Checks:{retries}   TimePassed:{TP}"
                )
...
def Join_multiple_combos():
    ...
    with open(
        os.getcwd() + "\\results\\JOINED_COMBOS" + namy,
        encoding="utf-8",
        errors="ignore",
    ) as file:
        for line in biglist:
...
def get_pagetypes_from_urls():
    ...
    with open(combo, "r", encoding="utf-8", errors="ignore") as urls:
        for line in urls.readlines():
            string = line
            match = re.search(
                r"([^\/\.]*)\.([^\/.]*|[^\/.]*\.|gov\.rw|com\.au|add\.YourOwnDommainExtentionsHereIfItHasMultipleDots|co\.za|ca\.us)\/[^\.]*[^\/]*\.([a-zA-Z1-9]*)\?([^=&]*)=",
                string,
            )
    ...
    with open(
        os.getcwd() + "\\results\\PGT_FROM_URLS\\" + namy,
        "a",
        encoding="utf8",
        errors="ignore",
    ) as file:
...
def login(username, password, HWID):
    req = requests.get(
        f"{website}" + f"login/{username.strip()}/{password.strip()}/{HWID}"
    )
    data = req.json()
    status = data["status"]
    if bool(status) == True:
...
def register(email, username, password, key, HWID):
    req = requests.get(
        f"{website}" + f"register/{key}/{email}/{username}/{password}/{HWID}"
    )
    data = req.json()
    status = data["status"]
    if status == True:
...
def get_combos():
    ...
    while pagenum < num:  # simple loop to scrape page source
        req = requests.get(f"https://sqli.cloud/t/combolists?page={pagenum}")
        src = req.text
    ...
    def newbest(newlist):
        for (
            ...
            newlist
        ):  # loop to print to user the 'newlist' and scraping outer uploadee links
            rein = requests.get(i)
            miaw = i[25 : len(i)]
            info = str(n) + ": " + miaw
            n = n + 1
    ...
    for url in lastdown:
        os.system("cls")
        ctypes.windll.kernel32.SetConsoleTitleW(
            f"|GOAT-ALLL IN ONE|  |VERSION|:3.8   |MODULE|:(Automatic Combo Dumper) Target: {target}  Load: {n}/{nums}  Quality: {random.choice(Qualities)}"
        )
        PrintGradient("#00FFFF", "#FF69B4", NAME)
        print(
            center(
        ...
        try:
            path = download(
                url,
                f"{os.getcwd()}\\results\\COMBO_DUMPER [{target}]\\combo_{target}_{n}.txt",
                replace=True,
                progressbar=True,
                verbose=False,
            )
...
def dorkbeg():
    ...
    def generate(r, time):
        ...
        with open(
            os.getcwd() + f"\\results\\DORK_MAKER\\{namy}", "a", errors="ignore"
        ) as f:
...
def auth():
    while True:
        ...
        if Choicee == "1":
            ...
            PrintGradient("#00FFFF", "#FF69B4", NAME)
            ctypes.windll.kernel32.SetConsoleTitleW(
                "|GOAT-ALLL IN ONE|  |VERSION|:3.8   |MODULE|:(Password-Edits)"
            )
            ...
            PrintGradient("#00FFFF", "#FF69B4", NAME)
            ctypes.windll.kernel32.SetConsoleTitleW(
                "|GOAT-ALLL IN ONE|  |VERSION|:3.8   |MODULE|:(Email-Edits)"
            )
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "|GOAT-ALLL IN ONE|  |VERSION|:3.8   |MODULE|:(Combo-Edits)"
            )
            ...
            PrintGradient("#00FFFF", "#FF69B4", NAME)
            ctypes.windll.kernel32.SetConsoleTitleW(
                "|GOAT-ALLL IN ONE|  |VERSION|:3.8   |MODULE|:(Dorking)"
            )
